 ...
 * @param {object} replyData - Data including tid and content.
 * @returns {Promise<object>} The new post data.
 */
export async function createReply(replyData) {
  const { tid, content } = replyData;
  try {
    console.log(chalk.blue(`   -> Posting reply to TID: ${tid}...`));
    const response = await apiClient.post(`/topics/${tid}`, { content });
...
export async function createTopic(topicData) {
  try {
    ...
    return response.data.response.topicData;
  } catch (error) {
