...
export async function findTopicByMetadata(encodedPath) {
  try {
    ...
    // The topic data from the search result is sufficient.
    // Fetching the full topic separately is only needed for the hash.
    const topicDetails = await apiClient.get(`/topics/${postData.topic.tid}`);
    return {
        tid: postData.topic.tid,
        slug: postData.topic.slug,
        title: postData.topic.title,
        mainPid: postData.topic.mainPid,
